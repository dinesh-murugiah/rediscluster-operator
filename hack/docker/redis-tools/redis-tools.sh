...
case "$op" in
  backup)
    ...
    redis-cli --rdb dump.rdb -h "${REDIS_HOST}" --user admin -a "${REDIS_PASSWORD}"
    redis-cli -h "${REDIS_HOST}" --user admin -a "${REDIS_PASSWORD}" CLUSTER NODES | grep myself > nodes.conf
    pwd
    ls -lh "$SOURCE_DIR"
